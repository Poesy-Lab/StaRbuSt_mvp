...
lastmod: 2025-04-30
---
# `Plot_Tank_Total_S_t.m` 문서

## 함수 개요

`Plot_Tank_Total_S_t` 함수는 주어진 `uiaxes` 핸들에 시간에 따른 탱크 내 총 엔트로피(S) 변화를 플롯합니다. 값은 자동으로 kJ/K 단위로 변환됩니다.
...
-   `y`: 시뮬레이션 결과 구조체 (`y.time`, `y.tank.S` 필드 필요).
    -   `y.time` (단위: s)
    -   `y.tank.S` (총 엔트로피, 단위: J/K)

## 설명

`ax`로 지정된 `uiaxes` 객체에 총 엔트로피 `y.tank.S`를 `y.time`에 대해 플롯합니다.

-   **플롯 스타일:** 파란색 실선 (`b-`), 선 굵기 1.5.
-   **단위 변환:** J/K -> kJ/K
-   **부가 기능:** 그리드 표시, 범례 표시, 축 레이블 및 제목 설정.
-   **동작 방식:** 새로운 Figure 창을 생성하지 않고 입력받은 `ax`에 직접 플롯합니다.
-   **참고:** 현재 총 엔트로피만 플롯합니다. (증기/액체 성분 플롯 기능은 코드 내 주석 처리됨)
...
axTotalS = uiaxes(tabTotalS, 'Units', 'normalized', 'Position', [0.07, 0.12, 0.88, 0.8]);
Plot_Tank_Total_S_t(axTotalS, y);
```
