...
lastmod: 2025-04-30
---
# `Plot_Tank_Spec_cp_t.m` 문서

## 함수 개요

`Plot_Tank_Spec_cp_t` 함수는 주어진 `uiaxes` 핸들에 시간에 따른 탱크 내 혼합물, 증기, 액체의 정압 비열(cp) 변화를 플롯합니다. 값은 자동으로 kJ/kg-K 단위로 변환됩니다.
...
-   `y`: 시뮬레이션 결과 구조체 (`y.time`, `y.tank.cp`, `y.tank.cp_v`, `y.tank.cp_l` 필드 필요).
    -   `y.time` (단위: s)
    -   `y.tank.cp` (혼합물 cp, 단위: J/kg-K)
    -   `y.tank.cp_v` (증기 cp, 단위: J/kg-K)
    -   `y.tank.cp_l` (액체 cp, 단위: J/kg-K)

## 설명

`ax`로 지정된 `uiaxes` 객체에 정압 비열(`cp`, `cp_v`, `cp_l`)을 `y.time`에 대해 플롯합니다.

-   **플롯 스타일:**
    -   혼합물 `cp`: 파란색 실선 (`b-`), 선 굵기 1.5
    -   증기 `cp_v`: 빨간색 파선 (`r--`)
    -   액체 `cp_l`: 검정색 점선 (`k:`)
...
axCp = uiaxes(tabCp, 'Units', 'normalized', 'Position', [0.07, 0.12, 0.88, 0.8]);
Plot_Tank_Spec_cp_t(axCp, y);
```
...
plot(ax, y.time, y.tank.cp_l / 1e3, 'k:', 'DisplayName', 'Liquid (cp_l)'); % kJ/kg-K, Liquid as dotted
hold(ax, 'off');
grid(ax, 'on');
xlabel(ax, 'Time (s)');
