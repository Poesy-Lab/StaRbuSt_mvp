...
lastmod: 2025-04-30
---
# `Plot_Tank_Spec_u_t.m` 문서

## 함수 개요

`Plot_Tank_Spec_u_t` 함수는 주어진 `uiaxes` 핸들에 시간에 따른 탱크 내 혼합물, 증기, 액체 비내부에너지 변화를 플롯합니다. 값은 자동으로 kJ/kg 단위로 변환됩니다.
...
-   `y`: 시뮬레이션 결과 구조체 (`y.time`, `y.tank.u`, `y.tank.u_v`, `y.tank.u_l` 필드 필요).
    -   `y.time` (단위: s)
    -   `y.tank.u` (혼합물 비내부에너지, 단위: J/kg)
    -   `y.tank.u_v` (증기 비내부에너지, 단위: J/kg)
    -   `y.tank.u_l` (액체 비내부에너지, 단위: J/kg)

## 설명

`ax`로 지정된 `uiaxes` 객체에 비내부에너지(`u`, `u_v`, `u_l`)를 `y.time`에 대해 플롯합니다.

-   **플롯 스타일:**
    -   혼합물 `u`: 파란색 실선 (`b-`), 선 굵기 1.5
    -   증기 `u_v`: 빨간색 파선 (`r--`)
    -   액체 `u_l`: 검정색 점선 (`k:`)
-   **단위 변환:** J/kg -> kJ/kg
...
plot(ax, y.time, y.tank.u_l / 1e3, 'k:', 'DisplayName', 'Liquid (u_l)'); % kJ/kg, Liquid as dotted
hold(ax, 'off');
grid(ax, 'on');
xlabel(ax, 'Time (s)');
