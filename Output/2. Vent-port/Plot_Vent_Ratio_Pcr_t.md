...
lastmod: 2025-04-30
---
# `Plot_Vent_Ratio_Pcr_t.m` 문서

## 함수 개요

`Plot_Vent_Ratio_Pcr_t` 함수는 주어진 `uiaxes` 핸들에 시간에 따른 벤트 포트의 임계 압력비(`P_amb / P_tank`의 초크 조건) 변화를 플롯합니다.
...
-   `y`: 시뮬레이션 결과 구조체 (`y.time`, `y.vent.ratio_Pcr` 필드 필요).
    -   `y.time` (단위: s)
    -   `y.vent.ratio_Pcr` (무차원)

## 설명

`ax`로 지정된 `uiaxes` 객체에 임계 압력비 `y.vent.ratio_Pcr`을 `y.time`에 대해 플롯합니다.

-   **플롯 스타일:** 자홍색 실선 (`m-`), 선 굵기 1.5.
