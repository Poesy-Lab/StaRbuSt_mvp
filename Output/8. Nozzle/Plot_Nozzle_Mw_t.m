function Plot_Nozzle_Mw_t(ax, y)
%Plot_Nozzle_Mw_t Plots nozzle molecular weight vs. time.
%
%   Inputs:
...
if isfield(y, 'time') && isfield(y, 'nozzle') && isfield(y.nozzle, 'mw')
    t = y.time;
    mw = y.nozzle.mw;
    ...
    if ~isempty(t_plot)
        ...
        title(ax, 'Nozzle Chamber Molecular Weight vs. Time');
        grid(ax, 'on');
        % legend(ax, 'Mw', 'Location', 'best'); % Optional legend
    else
        title(ax, 'Nozzle Mw vs. Time (No valid data to plot)');
    ...
    title(ax, 'Nozzle Mw vs. Time (Data Missing)');
    text(ax, 0.5, 0.5, sprintf('Missing required fields:\n%s%s', ...
                            iif(~isfield(y, 'time'), 'y.time ', ''), ...
                            iif(~isfield(y, 'nozzle') || ~isfield(y.nozzle, 'mw'), 'y.nozzle.mw ', '')), ...
