---
tags:
  - 플롯
  - 인젝터
  - 건도
  - 시각화
lastmod: 2025-04-30
---
# `Plot_Inj_X_t.m` 문서

## 함수 개요

`Plot_Inj_X_t` 함수는 주어진 `uiaxes` 핸들에 시간에 따른 인젝터 유체 건도(`X`) 변화를 플롯합니다.

```matlab
function Plot_Inj_X_t(ax, y)
```

## 입력값

-   `ax`: 그래프를 그릴 대상 `uiaxes` 핸들.
-   `y`: 시뮬레이션 결과 구조체 (`y.time`, `y.inj.X` 필드 필요).
    -   `y.time` (단위: s)
    -   `y.inj.X` (0~1 사이 값, 무차원)

## 설명

`ax`로 지정된 `uiaxes` 객체에 인젝터 건도 `y.inj.X`를 `y.time`에 대해 플롯합니다.

-   **플롯 스타일:** 청록색 실선 (`c-`), 선 굵기 1.5.
-   **부가 기능:** 그리드 표시, 축 레이블 및 제목 설정, y축 범위 설정 (-0.1 ~ 1.1).
-   **동작 방식:** 새로운 Figure 창을 생성하지 않고 입력받은 `ax`에 직접 플롯합니다.

## 사용 예시 (`Plot_Inj_Results.m` 내)

```matlab
% fig가 uifigure이고 tabGroup이 uitabgroup이라고 가정
tabX = uitab(tabGroup, 'Title', 'Quality');
axX = uiaxes(tabX, 'Units', 'normalized', 'Position', [0.07, 0.12, 0.88, 0.8]);
Plot_Inj_X_t(axX, y);
```

## 관련 항목 (See Also)

-   [[Plot_Inj_Results.m]] / [[Plot_Inj_Results.md]]
-   [[InjState_LiqFeed.m]]
-   [[InjState_VapFeed.m]]
-   MATLAB 함수: `uiaxes`, `plot`, `xlabel`, `ylabel`, `title`, `grid`, `ylim`

# 전체 코드

```MATLAB
function Plot_Inj_X_t(ax, y)
%Plot_Inj_X_t Plots the injector fluid quality (vapor mass fraction) vs. time.
%   Plots y.inj.X against y.time on the provided axes ax.
%
%   Inputs:
%       ax: Axes handle to plot on.
%       y: Simulation results structure (must contain y.time, y.inj.X).

plot(ax, y.time, y.inj.X, 'c-', 'LineWidth', 1.5); % Cyan solid line
grid(ax, 'on');
xlabel(ax, 'Time (s)');
ylabel(ax, 'Injector Quality (-)');
title(ax, 'Injector Fluid Quality vs Time');
ylim(ax, [-0.1, 1.1]); % Set y-axis limits slightly outside 0-1

end 