...
lastmod: 2025-04-30
---
# `Plot_Inj_Ratio_P_t.m` 문서

## 함수 개요

`Plot_Inj_Ratio_P_t` 함수는 주어진 `uiaxes` 핸들에 시간에 따른 인젝터 실제 압력비(`ratio_P` = P_downstream / P_injector) 변화를 플롯합니다.
...
-   `y`: 시뮬레이션 결과 구조체 (`y.time`, `y.inj.ratio_P` 필드 필요).
    -   `y.time` (단위: s)
    -   `y.inj.ratio_P` (무차원)

## 설명

`ax`로 지정된 `uiaxes` 객체에 인젝터 실제 압력비 `y.inj.ratio_P`를 `y.time`에 대해 플롯합니다.

-   **플롯 스타일:** 청록색 점선 (`c-.`), 선 굵기 1.5.
...
tabP = uitab(tabGroup, 'Title', 'Pressure Ratio');
axP = uiaxes(tabP, 'Units', 'normalized', 'Position', [0.07, 0.12, 0.88, 0.8]);
Plot_Inj_Ratio_P_t(axP, y);
```
