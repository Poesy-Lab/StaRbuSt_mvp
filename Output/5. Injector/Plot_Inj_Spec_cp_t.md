...
lastmod: 2025-04-30
---
# `Plot_Inj_Spec_cp_t.m` 문서

## 함수 개요

`Plot_Inj_Spec_cp_t` 함수는 주어진 `uiaxes` 핸들에 시간에 따른 인젝터 비열(정압) (혼합물 `cp`, 기체상 `cp_v`, 액체상 `cp_l`, kJ/kg-K) 변화를 함께 플롯합니다. 입력된 값(J/kg-K)을 kJ/kg-K 단위로 변환하여 표시합니다.
...
-   `y`: 시뮬레이션 결과 구조체 (`y.time`, `y.inj.cp`, `y.inj.cp_v`, `y.inj.cp_l` 필드 필요).
    -   `y.time` (단위: s)
    -   `y.inj.cp` (단위: J/kg-K)
    -   `y.inj.cp_v` (단위: J/kg-K)
    -   `y.inj.cp_l` (단위: J/kg-K)

## 설명

`ax`로 지정된 `uiaxes` 객체에 인젝터 혼합물, 기체상, 액체상 정압 비열 (J/kg-K)를 kJ/kg-K로 변환하여 `y.time`에 대해 함께 플롯합니다.
