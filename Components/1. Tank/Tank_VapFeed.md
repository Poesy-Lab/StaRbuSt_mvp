...
---
# 소개
- `Tank_VapFeed.m` 은 **탱크 내의 액체 상의 유출이 끝난 후 증기 상의 유출**이 일어나는 과정의 시스템을 모사한 함수이다.
...
- 탱크 하단 출구로 빠져나간 질량 유량이 존재해야 이 함수를 이용할 수 있다. 
...
| 탱크 유체 증기 상 비엔탈피 | $h_v$                   | x.tank.h_v  |                                  |
| 탱크 유체 총 엔탈피     | $H_{\text{tank}}$       | x.tank.H    |                                  |
| 탱크 출구 유출 질량 유량  | $\dot{m}_{out}$         | x.inj.mdot  |                                  |
...
# System
## 1. 탱크 유체 질량 및 밀도 갱신
- 1.1 벤트 포트 및 탱크 하단 출구에서 빠져나간 질량 유량 만큼 질량을 갱신함.
$$
m_{\text{tank}}(t+\Delta t)=m_{\text{tank}}(t) - \left(\dot{m}_{\text{vent}}(t)+\dot{m}_{\text{out}}(t)\right) \Delta t
$$
```MATLAB
...
- 2.2 탱크 하단 출구에서 빠져나간 **증기 상**의 질량 유량만큼 엔탈피 유량을 계산함.
$$
\dot{H}_{\text{out}}(t) = \dot{m}_{\text{out}}(t)h_{v}(t)
$$
```MATLAB
Hdot_out = mdot_out * hv;
```

- 2.3 탱크 평균 엔탈피를 갱신함.
$$
H_{\text{tank}}(t+\Delta t) = H_{\text{tank}}(t) - (\dot{H}_{\text{vent}}(t) + \dot{H}_{\text{out}}(t))\Delta t
$$
```MATLAB
...
if use_flash
    ...
    if Props.state == -1 || ~isfinite(Props.P)
        warning('Tank_VapFeed:FlashFail', 'GetPropsDH failed (rho=%.4g, h=%.4g). Falling back to lsqnonlin.', rho_tank, h_tank);
        use_flash = false;
    end
end
if ~use_flash
    ...
    % Call GetProps with state_input = 2 to force vapor state calculation
    Props = fluid.GetProps(T_tank, rho_tank, 2);
    % Props = fluid.GetProps(T_tank, rho_tank); % Call GetProps without state input
end
```
...
| 명칭         | 기호         | 출력 변수        | 단위    | 비고                               |
| ------------ | ---------- | ------------ | ----- | -------------------------------- |
| 상태 변수      | state, P, T, X | x.tank.state, P, T, X | -, Pa, K, - | 업데이트된 탱크 상태 (압력, 온도, 건도 등)<br>state는 항상 2, X는 항상 1.0 |
| 총 질량       | $m$        | x.tank.m     | kg    | 업데이트된 탱크 내부 총 질량                |
| 증기 질량     | $m_v$      | x.tank.m_v   | kg    | 업데이트된 증기 질량 ($m_v = m$)       |
| 액체 질량     | $m_l$      | x.tank.m_l   | kg    | 업데이트된 액체 질량 ($m_l = 0$)   |
...
```MATLAB
% 상태 변수
x.tank.state = Props.state; % Will be 2 from GetProps(..., 2)
x.tank.P = Props.P; % Pa
x.tank.T = Props.T; % K
x.tank.X = Props.X; % Always 1.0 from the forced value above
...
% 액체 및 증기 질량 업데이트 (X=1 이므로 m_l = 0, m_v = m)
x.tank.m_v = x.tank.m;     % 증기 질량 = 총 질량
x.tank.m_l = 0.0;          % 액체 질량 = 0
