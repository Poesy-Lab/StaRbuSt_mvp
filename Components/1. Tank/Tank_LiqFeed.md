...
---
# 소개
- `Tank_LiqFeed.m` 은 **런밸브 작동 후 탱크 내의 액체 상의 유출**이 일어나는 과정의 시스템을 모사한 함수이다.
...
- 벤트 포트에서 빠져나간 유체의 상태는 증기(기체)로 가정한다. 
	- 탱크 내부에 산화제를 충전할 때, 액체의 산화제가 탱크 내부 벤트 포트의 높이 만큼 채워지면 액체는 벤트포트로 빠져나간다. 즉, 벤트 포트 입구는 증기상의 산화제와 맞닿아 있게 된다.
...
| 탱크 유체 액체 상 비엔탈피 | $h_{\ell}$              | x.tank.h_l  |                                  |
...
- 2.2 탱크 하단 출구에서 빠져나간 **액체 상**의 질량 유량만큼 엔탈피 유량을 계산함.
$$
\dot{H}_{\text{out}}(t) = \dot{m}_{\text{out}}(t)h_{\ell}(t)
$$
```MATLAB
